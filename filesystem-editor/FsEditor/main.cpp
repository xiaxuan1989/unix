...
void usage(const char* hint = nullptr) {
    ...
    cout << "Unix 文件系统读写器" << endl;
    cout << endl;
    cout << "usage: fsedit imgFile option [imgsize]" << endl;
    ...
    cout << "> p [file path] [unix fs path]: 将文件写入unix文件系统。" << endl;
    cout << "> g [unix fs path] [file path]: 从文件系统取出文件。" << endl;
    ...
    cout << endl;
    cout << "路径使用 '|' 分隔。" << endl;
    cout << "example: > b |C://Program Files/soft/soft|" << endl;
}
    ...
    while (true) {
        ...
        if (operation == 'h') { // help
            ...
            if (!f.is_open()) {
                ...
                string unixFileName = readPath();
                fsAdapter.uploadFile(unixFileName, f);
                cout << "[info 5] 上传成功：" << unixFileName << endl;
            }

        } else if (operation == 'g') { // get

            string unixPath = readPath();
            string localPath = readPath();
            fstream f(localPath, ios::out | ios::binary);
            if (!f.is_open()) {
                cout << "[error 6] 无法打开：" << localPath << endl;
            } else {
                fsAdapter.downloadFile(unixPath, f);
                cout << "[info 7] 下载成功：" << unixPath << " -> " << localPath << endl;
            }
