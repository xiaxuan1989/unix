			...
			for(k = 0; k < FUNC; k++)	/* 枚举每根总线上的功能号(逻辑设备) */
			{
				...
				if( find == 1 )
				{
					...
					if(config[8] & 0x1)		/* Bit(0)为1表示端口地址为分离的I/O地址空间 */
					{
						...
						// we should enable Bus Master Function manually.
						//
						// see:
						//   https://www.alldatasheet.com/datasheet-pdf/pdf/66091/INTEL/82371SB.html
						// 
						auto value = (0x80000000 | (i << 16) | (j << 11) | (k << 8) | (0x04));
						IOPort::OutDWord(cfg_addr_port, value);
						IOPort::OutByte(cfg_data_port, 0x7);  // enable Bus Master Function
						// then, check the result.
						value = (0x80000000 | (i << 16) | (j << 11) | (k << 8) | (0x04));
						IOPort::OutDWord(cfg_addr_port, value);
						gConfig[1] = config[1] = IOPort::InDWord(cfg_data_port);
					}
					else	/* 否则为内存映射I/O地址空间 */
					{
